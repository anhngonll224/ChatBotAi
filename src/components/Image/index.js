...
function beforeUpload(file) {
  const isJpgOrPng = ['image/png', 'image/jpg', 'image/jpeg', 'image/bmp'].includes(file.type)
  if (!isJpgOrPng) {
...
export default class UploadImage extends React.Component {
  ...
  handleChange = async info => {
    ...
    if (info.file.status === 'done') {
      ...
      onGetImageInfo(imageUrl && imageUrl.split(',')[1], get(info.file.originFileObj, 'name'))
    }
  }
  ...
  renderImage = () => {
    ...
    return (
      <div className={styles.imageCnt}>
        ...
        {imageUrl && (
          <>
            <img className={styles.removeImg} src={removeImage} onClick={this.handleRemoveImage} />
